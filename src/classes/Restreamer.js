        ...
        https.get(url, (response) => {
            if (response.statusCode === 200) {
                response.on('data', (body) => {
                    const updateCheck = JSON.parse(body);
                    const updateAvailable = require('semver').lt(packageJson.version, updateCheck.restreamer.version);
